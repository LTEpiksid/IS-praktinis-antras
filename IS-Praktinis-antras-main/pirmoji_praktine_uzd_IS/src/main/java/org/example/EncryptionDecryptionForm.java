...
public class EncryptionDecryptionForm {
    ...
    private void encrypt(ActionEvent e) {
        try {
            ...
            // Get the cipher instance based on the selected mode
            Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");

            // If CBC mode is selected, get the IV (Initialization Vector) from the text field
            if ("CBC".equals(mode)) {
                ...
                if (ivBytes.length != 16) {
                    JOptionPane.showMessageDialog(frame, "Invalid IV size. Please provide an IV of 128 bits (16 bytes).", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
